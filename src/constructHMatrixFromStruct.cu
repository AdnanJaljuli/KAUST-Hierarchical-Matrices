...
void generateHMatrixFromStruct(unsigned int numberOfInputPoints, unsigned int bucketSize, unsigned int numSegments, unsigned int segmentSize, TLR_Matrix mortonOrderedMatrix, int ARA_R, float tolerance, HMatrix hierarchicalMatrix, unsigned int *maxRanks) {
    ...
    for(unsigned int level = hierarchicalMatrix.matrixStructure.numLevels - 2; level > 0; --level) {
        ...
        cudaMalloc((void**) &d_A, static_cast<uint64_t>(batchSize)*maxRows*maxRanks[hierarchicalMatrix.matrixStructure.numLevels - level - 2]*sizeof(H2Opus_Real));
        cudaMalloc((void**) &d_B, static_cast<uint64_t>(batchSize)*maxRows*maxRanks[hierarchicalMatrix.matrixStructure.numLevels - level - 2]*sizeof(H2Opus_Real));
        ...
        generateArrayOfPointersT<H2Opus_Real>(d_A, d_APtrs, maxRows*maxRanks[hierarchicalMatrix.matrixStructure.numLevels - level - 2], batchSize, 0);
        generateArrayOfPointersT<H2Opus_Real>(d_B, d_BPtrs, maxRows*maxRanks[hierarchicalMatrix.matrixStructure.numLevels - level - 2], batchSize, 0);
        ...
        int LRARAReturnVal = lr_kblas_ara_batch(kblasHandle, segmentSize, batchUnitSize, d_rowsBatch, d_colsBatch, tilePtrs.U, tilePtrs.V, d_scanRanksPtrs,
            ...
            tolerance, maxRows, maxCols, maxRanks[hierarchicalMatrix.matrixStructure.numLevels - level - 2], 16, ARA_R, randState, 0, batchSize
        );
        assert(LRARAReturnVal == 1);

        // allocate HMatrix level
        allocateAndCopyToHMatrixLevel(hierarchicalMatrix.levels[level - 1], d_ranks, hierarchicalMatrix.matrixStructure, level, d_A, d_B, maxRows, maxRanks[hierarchicalMatrix.matrixStructure.numLevels - level - 2]);
